...
export async function GET(req: Request) {
  ...
  const params: any[] = []
  let sql = `SELECT l.*, COUNT(q.id) AS quiz_count FROM lessons l LEFT JOIN quizzes q ON q.lesson_id = l.id`
  if (moduleId) { sql += ' WHERE l.module_id = ?'; params.push(moduleId) }
  sql += ' GROUP BY l.id ORDER BY l.module_id, l.order_index, l.id'
  const [rows] = await pool.execute<RowDataPacket[]>(sql, params)
  return NextResponse.json(rows)
}

export async function POST(req: Request) {
  const { moduleId, title, description, youtube_url, video_duration, order_index } = await req.json()
...
export async function PUT(req: Request) {
  const { id, title, description, youtube_url, video_duration, order_index } = await req.json()
  await pool.execute(
