...
export async function POST(req: Request) {
  try {
    const { module_id, title, description, content, youtube_url, duration_minutes, order_index } = await req.json()

    if (!module_id) return NextResponse.json({ error: 'Module ID required' }, { status: 400 })
    if (!title?.trim()) return NextResponse.json({ error: 'Title required' }, { status: 400 })

    const [result] = await pool.execute(
      `INSERT INTO lessons (module_id, title, description, content, youtube_url, duration_minutes, order_index)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [module_id, title, description || '', content || '', youtube_url || '', duration_minutes || 0, order_index || 0]
    )
