...
export async function GET(req: Request) {
  ...
  let sql = `SELECT q.*, GROUP_CONCAT(qq.id) as question_ids FROM quizzes q
             LEFT JOIN quiz_questions qq ON qq.quiz_id = q.id`
  const params: any[] = []
  if (moduleId) { sql += ' WHERE q.module_id = ?'; params.push(moduleId) }
  sql += ' GROUP BY q.id ORDER BY q.id'
  ...
  const quizzes = await Promise.all(quizRows.map(async (quiz) => {
    const [qRows] = await pool.execute<RowDataPacket[]>(
      'SELECT * FROM quiz_questions WHERE quiz_id = ? ORDER BY id', [quiz.id]
    )
...
export async function PUT(req: Request) {
  const { id, title, description, xpReward, timeLimit, color } = await req.json()
  await pool.execute(
    'UPDATE quizzes SET title=?, description=?, xp_reward=?, time_limit=?, color=? WHERE id=?',
    [title, description, xpReward, timeLimit, color, id]
  )
