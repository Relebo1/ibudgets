...
export async function POST() {
  // Only seed if modules table is empty
  ...
  for (const q of QUIZZES) {
    const moduleId = moduleIds[q.moduleIndex]
    const [qRes] = await pool.execute<ResultSetHeader>(
      ...
      [moduleId, q.title, q.description, q.xp_reward, q.time_limit, q.color]
    )
  ...
  return NextResponse.json({ message: 'Seeded successfully', modules: moduleIds.length })
}
