...
export default function LessonPage({ params }: { params: { moduleId: string; lessonId: string } }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/admin/lessons?moduleId=${params.moduleId}`).then(r => r.json()),
    ]).then(([modules, lessonsList]) => {
      const m = modules.find((x: any) => x.id === Number(params.moduleId))
      setModule(m)
      setLessons(lessonsList)
      const l = lessonsList.find((x: any) => x.id === Number(params.lessonId))
      setLesson(l)
      setLessonCompleted(l?.completed === 1)
      fetch(`/api/quizzes?userId=${user.id}&moduleId=${params.moduleId}`).then(r => r.json()).then(qs => {
        setQuizzes(qs.filter((q: any) => q.lesson_id === Number(params.lessonId)))
      })
    })
  }, [user, params])

  if (!user || !lesson || !module) return null

  const videoId = lesson.youtube_url ? getYouTubeId(lesson.youtube_url) : null
  const currentLesson = lessons.find(l => l.id === Number(params.lessonId))
  const lessonIndex = lessons.findIndex(l => l.id === Number(params.lessonId))
  ...
  const markLessonComplete = async () => {
    await fetch('/api/modules', {
      ...
      body: JSON.stringify({
        ...
        progress: Math.min(module.progress + 10, 100),
        completed: false,
        completedLessons: (module.completed_lessons || 0) + 1,
      }),
    })
            ...
            <div className={`p-4 rounded-xl mb-4 flex items-start gap-3 ${picked === q.correct_index ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800'}`}>
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-0.5">
                  {picked === q.correct_index ? '✓ Correct!' : `✗ Incorrect`}
                </p>
                {picked !== q.correct_index && (
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                    Correct answer: <span className="font-semibold">{String.fromCharCode(65 + q.correct_index)}. {q.options[q.correct_index]}</span>
                  </p>
                )}
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        <div className="card p-6 sm:p-8 text-center">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { setActiveQuiz(null); setFinished(false) }} className="btn-primary flex-1">Back to Lesson</button>
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={() => window.history.back()} className="text-sm text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 flex items-center gap-1.5 bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-lg transition-colors">
      ...
      <div className="card p-6">
        <div className="flex items-start gap-4 mb-4">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 bg-brand-50 dark:bg-brand-900/30">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 text-xs text-gray-400 mt-2">
              <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {module.title}</span>
              {lesson.video_duration && (
                <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {lesson.video_duration}</span>
              )}
            </div>
          </div>
          {lessonCompleted && (
            <span className="badge bg-green-50 text-green-700 dark:bg-green-900/40 dark:text-green-400 flex items-center gap-1 flex-shrink-0">
      ...
      {/* Mark complete */}
      {!lessonCompleted && (
        <button onClick={markLessonComplete} className="btn-primary w-full flex items-center justify-center gap-2">
          <CheckCircle className="w-4 h-4" /> Mark Lesson as Complete
        </button>
      )}
